...
from image_dataset import ImagePairDataset
from ImageEmb import PlaceImageEmb

# Configuration
os.environ['torch_ljl'] = '/home'

data_dir = "/visual" # Path to the directory containing images
train_path_list_path = './pair-train.pkl'
val_path_list_path = './pair-test.pkl'

CNN_model_path = './inception_v3_google-1a9a5a14.pth'
old_ckpt_path = None
ckpt_save_dir = '' # saved model path
...
if __name__ == '__main__':
    ...
    with open(val_path_list_path, 'rb') as w:
        val_path_list = pickle.load(w)#[0:32]

    print('training set size: ' + str(len(train_path_list)))
    print('validation set size: ' + str(len(val_path_list)))


    datasets = {'train': ImagePairDataset(data_dir, train_path_list, data_transforms['train']),
                'val': ImagePairDataset(data_dir, val_path_list, data_transforms['val'])}
    ...
    for i in learning_rate:
        for j in margin: 
            ...
            model.load_CNN_params(CNN_model_path, device=torch.device('cpu'))
            ...
            if not trainable_params == None:
                model.only_train(trainable_params=trainable_params)  
